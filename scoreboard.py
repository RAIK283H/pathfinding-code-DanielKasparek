...
class Scoreboard:
    ...
    def __init__(self, batch, group):
        ...
        for index, player in enumerate(config_data.player_data):
            ...
            self.player_path_display.append(
                (path_label, player))
            num_nodes_visited_label = pyglet.text.Label("Number of Nodes Visited:",
                                                        x=0,
                                                        y=0,
                                                        font_name='Arial',
                                                        font_size=self.font_size, batch=batch, group=group, color=player[2][colors.TEXT_INDEX])
            self.player_nodes_visited_display.append(
                (num_nodes_visited_label, player))
    ...
    def update_winner(self):
        ...
        for index, player in enumerate(global_game_data.player_objects):
            if player.distance_traveled < shortest_distance:
                shortest_distance = player.distance_traveled
                self.winner = config_data.player_data[index][0]
    ...
    def update_num_nodes_in_path(self):
        """
        Updates the number of nodes visited for each player.
        """
        for index in range(len(config_data.player_data)):
            num_nodes_visited = len(set(global_game_data.graph_paths[index]))  # Use set to avoid counting duplicates
            self.player_nodes_visited_display[index][0].text = f"Unique Nodes Visited: {num_nodes_visited}"
